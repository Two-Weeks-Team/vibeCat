...
public enum NavigatorVoiceCommandDetector {
    public static func shouldRoute(
        ...
        var executeScore = keywordScore(lowered, keywords: [
            ...
            "반영", "적용", "실행", "다시 돌려", "다시 실행", "수정", "해결", "처리해", "눌러", "클릭", "입력", "붙여넣", "써", "선택", "포커스", "볼륨", "음량", "음소거", "소리"
        ])
        ...
        if hasVisibleTextInput(context) && referencesCurrentTarget(lowered) &&
            containsAny(lowered, ["입력", "붙여넣", "써", "type", "enter", "paste", "fill", "write"]) {
            executeScore = max(executeScore, 0.84)
        }
    ...
    private static func hasTextEntryPayload(_ command: String) -> Bool {
        ...
        for marker in ["입력해줘:", "입력해:", "입력:", "붙여넣어:", "붙여넣기:", "type:", "enter:", "paste:", "fill:"] {
